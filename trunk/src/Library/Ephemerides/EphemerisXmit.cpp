...
// EphemerisXmit implements the ephemeris as transmitted by the satellites
...
EphemerisXmit::~EphemerisXmit(void)
{
}

bool EphemerisXmit::Valid(Time t)
{
	if (MaxTime > 0 && (t < MinTime || t > MaxTime))
		debug("EphemerisXmit - No longer valid\n s=%d  t=%.0f Min=%.0f Max=%.0f\n",
		   SatIndex, S(t), S(MinTime), S(MaxTime) );
    return (t >= MinTime && t <= MaxTime);
}


double EphemerisXmit::Accuracy(Time t)
{
...
bool::EphemerisXmit::AddFrame(Frame& f)
{
...
bool EphemerisXmit::GetFrame(Frame& f)
{
	// Clear the frame
	f.Init(22);
